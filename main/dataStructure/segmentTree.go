...
func (st *SegmentTree) buildSegmentTree(treeIndex, left, right int) {
	...
	midIndex, leftTreeIndex, rightTreeIndex := left+(right-left)>>1, st.leftChild(treeIndex), st.rightChild(treeIndex)
	st.buildSegmentTree(leftTreeIndex, left, midIndex)
	st.buildSegmentTree(rightTreeIndex, midIndex+1, right)
...
func (st *SegmentTree) queryInTree(treeIndex, left, right, queryLeft, queryRight int) int {
	...
	if queryLeft > midIndex {
		return st.queryInTree(rightTreeIndex, midIndex+1, right, queryLeft, queryRight)
	} else if queryRight <= midIndex {
		return st.queryInTree(leftTreeIndex, left, midIndex, queryLeft, queryRight)
	}
	return st.merge(st.queryInTree(leftTreeIndex, left, midIndex, queryLeft, midIndex),
		st.queryInTree(rightTreeIndex, midIndex+1, right, midIndex+1, queryRight))
}
...
// 计数线段树：https://leetcode-cn.com/problems/count-of-range-sum/solution/qu-jian-he-de-ge-shu-by-leetcode-solution/
